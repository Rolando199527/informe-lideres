...
try {
  const res = await fetch("http://localhost:5432//coordinaciones");
...
function obtenerCoordinacion(lider) {
  for (const [coordinacion, lideres] of Object.entries(
    ESTRUCTURA.COORDINACIONES
  )) {
...
document.getElementById("informeForm").addEventListener("submit", function (e) {
  ...
  const data = {
    ...
    asistenciaIglesia: {
      miercoles:
        parseInt(document.getElementById("iglesiaMiercoles").value) || 0,
    ...
    nuevosDiscipulos:
      document.getElementById("nuevosDiscipulos").value.trim() === ""
        ? []
        : document
            .getElementById("nuevosDiscipulos")
            .value.split(",")
            .map((n) => n.trim())
            .filter((n) => n !== ""),
  };
  ...
  mostrarNotificacion(
    "WAAOOO Líder! Informe guardado correctamente",
    "success"
  );
...
function cargarInformes() {
  ...
  const coordinaciones = [
    ...new Set(informes.map((i) => i.coordinacion).filter(Boolean)),
  ];
  filtro.innerHTML = `
        <option value="todas">Todas</option>
        <option value="sin_informe">Quién no llenó su informe</option>
...
function filtrarInformes() {
  ...
  if (filtro === "sin_informe") {
    ...
    const filtrados =
      filtro === "todas"
        ? informes
        : informes.filter((i) => i.coordinacion === filtro);
...
function mostrarLideresSinInforme() {
  ...
  const lideresSinInforme = TODOS_LIDERES.filter(
    (lider) => !lideresConInforme.includes(lider)
  );

  contenedor.innerHTML = `
        <div class="info-box">
            <h3><i class="fas fa-exclamation-triangle"></i> Líderes que no han llenado informes</h3>
            <ol>${lideresSinInforme
              .map((lider) => `<li><strong>${lider}</strong></li>`)
              .join("")}</ol>
        </div>
    `;
  ...
  informes.forEach((inf) => {
    ...
    div.innerHTML = `
            <div class="informe-header">
                <h4>${inf.lider}</h4>
                <span class="informe-meta">${inf.coordinacion} • ${formatFecha(
      inf.fecha
    )}</span>
            </div>
            ...
            <div class="informe-content">
                ...
                <div class="informe-row ofrenda">
                    ...
                    <span class="informe-value">$${parseFloat(
                      inf.asistenciaRedes.ofrenda || 0
                    ).toFixed(2)}</span>
                </div>
                ...
                <div class="informe-row">
                    ...
                    <span class="informe-value">${inf.nuevosDiscipulos.join(
                      ", "
                    )}</span>
                </div>
                `
        ...
        <div class="totales-box">
            ...
            <div class="totales-grid">
                ...
                <div class="totales-item">
                    ...
                    <span class="totales-value ofrenda">$${totalOfrenda.toFixed(
                      2
                    )}</span>
                </div>
...
function exportarExcel() {
  ...
  const rows = informes.map((inf, index) => {
    ...
    const totalRedesLider =
      parseInt(inf.asistenciaRedes.discipulos || 0) +
      parseInt(inf.asistenciaRedes.nuevos || 0);
  ...
  link.download = `Informes_Lideres_${
...
function borrarInformes() {
  if (
    confirm(
      "¿Estás seguro de borrar todos los informes? Esta acción no se puede deshacer."
    )
  ) {
